...
\begin{tikzpicture}[line width=0.2mm]
	...
	\fill[gray!40] (4,6)-- (6,6) -- (6,4) -- cycle;
	\fill[pattern=north east lines, pattern color=black!60] (4,6)-- (6,6) -- (6,4) -- cycle;
	...
	\draw[black, thick, {Latex}-]
	(6,4) ++(-\rstart,\rstart) -- ++(-\rlen,\rlen)
	node[midway, above left,xshift=-2pt, yshift=-7pt] {$\bm{c}_8$};
	...
	%\draw[red,fill=red] (8,4)node[]{} circle(0.1cm);%1
	\node[draw, black, fill=white, rectangle, minimum size=0.18cm, inner sep=0pt] at (6,2) {};%2
	\node[draw, black, fill=white, rectangle, minimum size=0.18cm, inner sep=0pt] at (4,4) {};%3
	%\draw[gray,fill=gray] (6,6)node[]{} circle(0.1cm);%4
	%\draw[gray,fill=gray] (8,6)node[]{} circle(0.1cm);%5
	\draw[black,fill=black] (4,6)node[]{} circle(0.1cm);%6
