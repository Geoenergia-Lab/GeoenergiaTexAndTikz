...
\begin{tikzpicture}[line width=0.2mm]
	...
	\fill[gray!20] (4,4) -- (4,2) -- (6,2)  -- cycle;
	\fill[pattern=north east lines, pattern color=black!60](4,4) -- (4,2) -- (6,2)  -- cycle;

	\draw[solid,thin,line width=0.2mm, black!50 ] (4,4) -- (6,2);
	%\fill[gray!40] (4,4) -- (8,4) -- (8,6) -- (4,6) -- cycle;
	%\fill[gray!40] (6,2) -- (8,2) -- (8,4) -- (6,4) -- cycle;

	%\draw[dashed,thin,line width=0.1mm] (3.5,4) -- (8.5,4);

	\draw[solid,thin,line width=0.2mm, gray!20] (6,2) -- (8,2);
	...
	\draw[solid,thin,line width=0.2mm, gray!20] (4,4) -- (4,6);
	...
	\draw[black, thick, {Latex}-]
	(6,4) ++(-\rstart,0) -- ++(-\rlen,0)
	node[midway, above, yshift=-2pt,font=\fontsize{14pt}{12pt}\selectfont] {$\bm{c}_1$};

	% down (4)
	\draw[black, thick, {Latex}-]
	(6,4) ++(0,-\rstart) -- ++(0,-\rlen)
	node[midway, right, xshift=-2pt,font=\fontsize{14pt}{12pt}\selectfont] {$\bm{c}_2$};
	...
	\draw[black,fill=black] (6,2)node[]{} circle(0.1cm);%2
	\draw[black,fill=black] (4,4)node[]{} circle(0.1cm);%3
	\draw[black,fill=white] (6,6)node[]{} circle(0.1cm);%4
	...
	\draw (6,1.5)node[]{$I_s = \{0,1,2,3,4,6,7,8\}$};
	%\draw (6,1)node[]{$O_s = \{0,1,2,3,4,5,6,8\}$};
