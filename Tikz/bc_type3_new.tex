...
\fill[gray!40] (6,4) -- (8,4) -- (8,6) -- (6,6) -- cycle;
\fill[pattern=north west lines, pattern color=black!60] (6,4) -- (8,4) -- (8,6) -- (6,6) -- cycle;
...
%(6,4) ++(\rstart,0) -- ++(\rlen,0)
%node[midway,below, xshift=-5pt, yshift=1pt] {$c_3$};
...
\draw[black, thick, {Latex}-]
(6,4) ++(0,\rstart) -- ++(0,\rlen)
node[midway, left, xshift=4pt] {$\bm{c}_4$};
...
\draw[black, thick, {Latex}-]
(6,4) ++(-\rstart,\rstart) -- ++(-\rlen,\rlen)
node[midway, above left,xshift=4pt, yshift=7pt] {$\bm{c}_8$};
